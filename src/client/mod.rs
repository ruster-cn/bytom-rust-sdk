...
use crate::key::{Key, CheckPasswordResp, ResetPasswordResp, Token};
use crate::client::response::Response;
use crate::error::BtmError;
...
use crate::account::response::{Receiver, AddressResp, ValidateAddressResp, MiningAddressResp, AccountPubkey, AccountBalance, GetWorkJSONResp};
use crate::transaction::response::{CoinbaseArbitrary, AnnotateAsset, AnnotatedUTXO, SignMsgResp, DecodeProgResp, Template, SignTemplateResp, SignTemplatesResp, SubmitTxResp, SubmitTxsResp, EstimateTxGasInfo, UnconfirmedTxsResp, GetBlockResp, GetBlockHeaderResp, GetDifficultyResp, GetHashRateResp, VerifyMsgResp, ComileResp, GetWorkResp};
use crate::transaction::request::{CreateAssetRequestParam, ListUnspentOutputsRequestParam, ListTransactionsRequestParam, BuildTransactionsRequestParam, SignTemplateRequestParam, SignTemplatesRequestParam, CompileReq, SubmitWorkJSONReq};
use crate::wallet::{WalletImage, WalletInfo};
use crate::transaction::{AnnotatedTx, TransactionFeed, BlockTx, RawTx};
use std::collections::HashMap;
use crate::node::{VersionInfo, PeerInfo};
...
impl BtmClient {
    ...
    pub async fn create_key(
        ...
        if response.is_success() {
            ...
            Err(Box::new(BtmError::new(response)))
        }
    }

    async fn list_keys_bytes(&self) -> Result<Bytes, Box<dyn Error>> {
        let response_byte = self
            .post(String::from("/list-keys"), String::new())
            .await?;
        Ok(response_byte)
    }

    pub async fn list_keys(&self) -> Result<Vec<Key>,Box<dyn Error>>{
    ...
    async fn update_key_alias_bytes(
        &self,
        xpub: &str,
        new_alias: &str,
    ) -> Result<Bytes, Box<dyn Error>> {
        ...
        println!("{}",data);
        let response_byte = self
            .post(String::from("/update-key-alias"), data)
            .await?;
        Ok(response_byte)
    }

    pub async fn update_key_alias(
        &self,
        xpub: &str,
        new_alias: &str,
    ...
    async fn delete_key_bytes(&self, xpub: &str, password: &str) -> Result<Bytes, Box<dyn Error>> {
        let data = format!(r#"{{"xpub":"{}","password":"{}"}}"#, xpub, password);
        let response_byte = self.post(String::from("/delete-key"), data).await?;
        Ok(response_byte)
    }

    pub async fn delete_key(&self, xpub: &str, password: &str) -> Result<bool,Box<dyn Error>>{
    ...
    async fn check_key_password_bytes(
        &self,
        xpub: &str,
        password: &str,
    ) -> Result<Bytes, Box<dyn Error>> {
        let data = format!(r#"{{"xpub":"{}","password":"{}"}}"#, xpub, password);
        let response_byte = self
            .post(String::from("/check-key-password"), data)
            .await?;
        Ok(response_byte)
    }

    pub async fn check_key_password(
        &self,
        xpub: &str,
        password: &str,
    )-> Result<bool,Box<dyn Error>>{
    ...
    async fn reset_key_password_bytes(
        ...
        let response_byte = self
            .post(String::from("/reset-key-password"), data)
            .await?;
        Ok(response_byte)
    }
    ...
    pub async fn reset_key_password(
        ...
        let response_byte = self.reset_key_password_bytes(xpub,old_password,new_password).await?;
        println!("{:?}",str::from_utf8(response_byte.bytes()));
    ...
    async fn create_account_bytes(
        ...
        let response_byte = self
            .post(String::from("/create-account"), data)
            .await?;
        Ok(response_byte)
    }
    ...
    pub async fn create_account(
        &self,
        create_account_request_param: &CreateAccountRequestParam
    ) -> Result<AnnotatedAccount,Box<dyn Error>>{
        let response_byte = self.create_account_bytes(create_account_request_param).await?;
        println!("{:?}",str::from_utf8(response_byte.bytes()));
        let result: Response<AnnotatedAccount> = serde_json::from_slice(response_byte.bytes())?;
        if result.is_success() {
            Ok(result.data)
        } else {
            Err(Box::new(BtmError::new(result)))
        }
    }

    async fn list_account_bytes(&self, id: &str, alias: &str) -> Result<Bytes, Box<dyn Error>> {
        let data = format!(r#"{{"alias":"{}","id":"{}"}}"#, alias, id);
        let response_byte = self
            .post(String::from("/list-accounts"), data)
            .await?;
        Ok(response_byte)
    }

    pub async fn list_account(&self, id: &str, alias: &str) -> Result<Vec<AnnotatedAccount>,Box<dyn Error>>{
        let response_byte = self.list_account_bytes(id,alias).await?;
        println!("{:?}",str::from_utf8(response_byte.bytes()));
        let result: Response<Vec<AnnotatedAccount>> = serde_json::from_slice(response_byte.bytes())?;
        if result.is_success() {
            Ok(result.data)
        } else {
            Err(Box::new(BtmError::new(result)))
    ...
    pub async fn update_account_alias(
        ...
        let response_byte = self.update_account_alias_bytes(new_alias,old_alias,id).await?;
        println!("{:?}",str::from_utf8(response_byte.bytes()));
        let result: Response<String> = serde_json::from_slice(response_byte.bytes())?;
       Ok(result.is_success())
    }

    async fn delete_account_bytes(&self, alias: &str, id: &str) -> Result<Bytes, Box<dyn Error>> {
        let data = format!(r#"{{"account_id":"{}","account_alias":"{}"}}"#, id, alias);
        let response_byte = self
            .post(String::from("/delete-account"), data)
            .await?;
        Ok(response_byte)
    }

    pub async fn delete_account(&self,alias:&str,id:&str) -> Result<bool,Box<dyn Error>>{
    ...
    async fn create_account_receiver_bytes(
        &self,
        alias: &str,
        id: &str,
    ) -> Result<Bytes, Box<dyn Error>> {
    ...
    pub async fn create_account_receiver(
        &self,
        alias: &str,
        id: &str,
    ...
    async fn list_address_bytes(
        ...
        let response_byte = self
            .post(String::from("/list-addresses"), data)
            .await?;
        Ok(response_byte)
    }
    ...
    pub async fn list_address(
        ...
        from: u32,
        count: u32,
    ) -> Result<Vec<AddressResp>,Box<dyn Error>>{
    ...
    async fn validate_address_bytes(&self, address: &str) -> Result<Bytes, Box<dyn Error>> {
        let data = format!(r#"{{"address":"{}"}}"#, address);
        let response_byte = self
            .post(String::from("/validate-address"), data)
            .await?;
        Ok(response_byte)
    }

    pub async fn validate_address(&self, address: &str) -> Result<ValidateAddressResp,Box<dyn Error>>{
    ...
    async fn get_mining_address_bytes(&self) -> Result<Bytes, Box<dyn Error>> {
        let response_byte = self
    ...
    pub async fn get_mining_address(&self) -> Result<String,Box<dyn Error>>{
    ...
    async fn set_mining_address_bytes(&self, address: &str) -> Result<Bytes, Box<dyn Error>> {
        let data = format!(r#"{{"mining_address":"{}"}}"#, address);
        let response_byte = self
            .post(String::from("/set-mining-address"), data)
            .await?;
        Ok(response_byte)
    }


    pub async fn set_mining_address(&self, address:&str) -> Result<bool,Box<dyn Error>>{
    ...
    async fn get_coinbase_arbitrary_bytes(&self) -> Result<Bytes, Box<dyn Error>> {
        let response_byte = self
    ...
    pub async fn get_coinbase_arbitrary(&self) ->Result<String,Box<dyn Error>>{
    ...
    async fn set_coinbase_arbitrary_bytes(&self, arbitrary: &str) -> Result<Bytes, Box<dyn Error>> {
    ...
    pub async fn set_coinbase_arbitrary(&self,arbitrary: &str) -> Result<bool,Box<dyn Error>>{
    ...
    async fn list_pubkeys_bytes(
        ...
        let response_byte = self
            .post(String::from("/list-pubkeys"), data)
            .await?;
        Ok(response_byte)
    }
    ...
    pub async fn list_pubkeys(
        ...
        id: &str,
        pubkey: &str,
    ) -> Result<AccountPubkey,Box<dyn Error>>{
    ...
    pub async fn create_asset(
        &self,
        param: &CreateAssetRequestParam
    )->Result<AnnotateAsset,Box<dyn Error>>{
    ...
    async fn get_asset_bytes(&self, id: &str) -> Result<Bytes, Box<dyn Error>> {
        let data = format!(r#"{{"id":"{}"}}"#, id);
        let response_byte = self.post(String::from("/get-asset"), data).await?;
        Ok(response_byte)
    }

    pub async fn get_asset(&self, id:&str) -> Result<AnnotateAsset,Box<dyn Error>>{
    ...
    async fn list_assets_bytes(&self,id: &str) -> Result<Bytes, Box<dyn Error>> {
        let data = format!(r#"{{"id":"{}"}}"#, id);
        let response_byte = self
            .post(String::from("/list-assets"), data)
            .await?;
        Ok(response_byte)
    }

    //todo the field of AnnotateAsset definition must support dynamic typing.
    pub async fn list_assets(&self,id: &str)  ->Result<Vec<AnnotateAsset>,Box<dyn Error>>{
    ...
    async fn update_asset_alias_bytes(&self, id: &str, alias: &str) -> Result<Bytes, Box<dyn Error>> {
        let data = format!(r#"{{"id":"{}","alias":"{}"}}"#, id, alias);
        let response_byte = self
            .post(String::from("/update-asset-alias"), data)
            .await?;
        Ok(response_byte)
    }

    pub async fn update_asset_alias(&self, id: &str, alias: &str) -> Result<bool, Box<dyn Error>>{
    ...
    async fn list_balances_bytes(
        ...
        let response_byte = self
            .post(String::from("/list-balances"), data)
            .await?;
        Ok(response_byte)
    }
    ...
    pub async fn list_unspent_outputs(
        &self,
        param: &ListUnspentOutputsRequestParam
    )->Result<Vec<AnnotatedUTXO>,Box<dyn Error>>{
    ...
    async fn backup_wallet_bytes(&self) -> Result<Bytes, Box<dyn Error>> {
        let response_byte = self
    ...
    pub async fn backup_wallet(&self) -> Result<WalletImage,Box<dyn Error>>{
    ...
    pub async fn restore_wallet(&self,image: &WalletImage) -> Result<bool,Box<dyn Error>>{
    ...
    pub async fn rescan_wallet(&self) -> Result<bool, Box<dyn Error>> {
        let response_byte = self
    ...
    pub async fn recovery_wallet(&self, xpubs: &Vec<String>) -> Result<bool, Box<dyn Error>> {
        let data = serde_json::to_string(xpubs)?;
        let response_byte = self
            .post(String::from("/recovery-wallet"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
        let result:Response<String> = serde_json::from_slice(response_byte.bytes())?;
        Ok(result.is_success())
    ...
    pub async fn wallet_info(&self) -> Result<WalletInfo, Box<dyn Error>> {
        let response_byte = self
    ...
    pub async fn sign_message(
        ...
        message: &str,
        password: &str,
    ) -> Result<SignMsgResp, Box<dyn Error>> {
        let data = format!(
            r#"{{"address":"{}","message":"{}","password":"{}"}}"#,
            address, message, password
        );
        let response_byte = self
            .post(String::from("/sign-message"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn decode_message(&self, program: &str) -> Result<DecodeProgResp, Box<dyn Error>> {
        let data = format!(r#"{{"program":"{}"}}"#, program);
        let response_byte = self
            .post(String::from("/decode-program"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn get_transaction(&self, tx_id: &str) -> Result<AnnotatedTx, Box<dyn Error>> {
        let data = format!(r#"{{"tx_id":"{}"}}"#, tx_id);
        let response_byte = self
            .post(String::from("/get-transaction"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    ) -> Result<Vec<AnnotatedTx>, Box<dyn Error>> {
       let data = serde_json::to_string(param)?;
        let response_byte = self
            .post(String::from("/list-transactions"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn build_transaction(&self,param: &BuildTransactionsRequestParam) -> Result<Template,Box<dyn Error>>{
        let data = serde_json::to_string(param)?;
        let response_byte = self
            .post(String::from("/build-transactions"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn build_chain_transactions(&self,param: &BuildTransactionsRequestParam) -> Result<Template, Box<dyn Error>>{
        let data = serde_json::to_string(param)?;
        let response_byte = self
    ...
    pub async fn sign_transaction(&self,param: &SignTemplateRequestParam) -> Result<SignTemplateResp, Box<dyn Error>>{
        let data = serde_json::to_string(param)?;
        let response_byte = self
            .post(String::from("/sign-transaction"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn sign_transactions(&self,param: &SignTemplatesRequestParam) -> Result<SignTemplatesResp, Box<dyn Error>>{
        let data = serde_json::to_string(param)?;
        let response_byte = self
            .post(String::from("/sign-transactions"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn submit_transaction(
        &self,
        param: &Template,
    ) -> Result<SubmitTxResp, Box<dyn Error>> {
        let data = serde_json::to_string(param)?;
        let response_byte = self
            .post(String::from("/submit-transaction"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn submit_transactions(
        &self,
        param: &Vec<Template>,
    ) -> Result<SubmitTxsResp, Box<dyn Error>> {
        let data = serde_json::to_string(param)?;
        let response_byte = self
    ...
    pub async fn estimate_transaction_gas(&self,param: &Template) -> Result<EstimateTxGasInfo,Box<dyn Error>>{
        let data = serde_json::to_string(param)?;
        let response_byte = self
    ...
    pub async fn list_access_tokens(&self) -> Result<Vec<Token>, Box<dyn Error>> {
        let response_byte = self
    ...
    pub async fn delete_access_token(&self, id: &str) -> Result<bool, Box<dyn Error>> {
        let data = format!(r#"{{"id":"{}"}}"#, id);
        let response_byte = self
    ...
    pub async fn check_access_token(
        ...
        let response_byte = self
            .post(String::from("/check-access-token"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
        let result:Response<String> = serde_json::from_slice(response_byte.bytes())?;
        Ok(result.is_success())
    ...
    pub async fn create_transaction_feed(
        &self,
        alias: &str,
        filter: &str,
    ) -> Result<bool, Box<dyn Error>> {
    ...
    pub async fn get_transaction_feed(&self, alias: &str) -> Result<TransactionFeed, Box<dyn Error>> {
    ...
    pub async fn list_transaction_feeds(&self) -> Result<Vec<TransactionFeed>, Box<dyn Error>> {
        let response_byte = self
    ...
    pub async fn delete_transaction_feed(&self, alias: &str) -> Result<bool, Box<dyn Error>> {
        let data = format!(r#"{{"alias":"{}"}}"#, alias);
        let response_byte = self
    ...
    pub async fn update_transaction_feed(
        &self,
        alias: &str,
        filter: &str,
    ) -> Result<bool, Box<dyn Error>> {
    ...
    pub async fn get_unconfirmed_transaction(&self, tx_id: &str) -> Result<BlockTx, Box<dyn Error>> {
        let data = format!(r#"{{"tx_id":"{}"}}"#, tx_id);
        let response_byte = self
    ...
    pub async fn list_unconfirmed_transaction(&self)  -> Result<UnconfirmedTxsResp, Box<dyn Error>> {
        let response_byte = self
            .post(String::from("/list-unconfirmed-transactions"), String::new())
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn get_block_hash(&self) -> Result<String, Box<dyn Error>> {
        let response_byte = self
            .post(String::from("/get-block-hash"), String::new())
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
        let result: Response<HashMap<String,String>> = serde_json::from_slice(response_byte.bytes())?;
        if result.is_success(){
    ...
    pub async fn get_block(&self, height: u64, hash: &str) -> Result<GetBlockResp, Box<dyn Error>> {
    ...
    pub async fn get_block_header(&self, hash: &str, height: u64) -> Result<GetBlockHeaderResp, Box<dyn Error>> {
        let data = format!(r#"{{"block_height":{},"block_hash":"{}"}}"#, height, hash);
        let response_byte = self
            .post(String::from("/get-block-header"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn get_difficulty(&self, hash: &str, height: u64) -> Result<GetDifficultyResp, Box<dyn Error>> {
        let data = format!(r#"{{"block_height":{},"block_hash":"{}"}}"#, height, hash);
        let response_byte = self
            .post(String::from("/get-difficulty"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn get_hash_rate(&self, hash: &str, height: u64) -> Result<GetHashRateResp, Box<dyn Error>> {
        let data = format!(r#"{{"block_height":{},"block_hash":"{}"}}"#, height, hash);
        let response_byte = self
            .post(String::from("/get-hash-rate"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn net_info(&self) -> Result<VersionInfo, Box<dyn Error>> {
        let response_byte = self
            .post(String::from("/net-info"), String::new())
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn is_mining(&self) -> Result<bool, Box<dyn Error>> {
        let response_byte = self
            .post(String::from("/is-mining"), String::new())
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
        let result: Response<HashMap<String,bool>> = serde_json::from_slice(response_byte.bytes())?;
        if result.is_success(){
    ...
    pub async fn set_mining(&self, is_mining: bool) -> Result<bool, Box<dyn Error>> {
    ...
    pub async fn gas_rate(&self) -> Result<u64, Box<dyn Error>> {
        let response_byte = self
            .post(String::from("/gas-rate"), String::new())
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
        let result: Response<HashMap<String,u64>> = serde_json::from_slice(response_byte.bytes())?;
        if result.is_success(){
    ...
    pub async fn verify_message(
        ...
        let response_byte = self
            .post(String::from("/verify-message"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn compile(&self,param: &CompileReq) -> Result<ComileResp,Box<dyn Error>>{
        let data = serde_json::to_string(param)?;
        let response_byte = self
            .post(String::from("/compile"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn list_peers(&self) -> Result<Vec<PeerInfo>, Box<dyn Error>> {
        let response_byte = self
    ...
    pub async fn disconnect_peer(&self, peer_id: &str) -> Result<bool, Box<dyn Error>> {
    ...
    pub async fn connect_peer(&self, ip: &str, port: u16) -> Result<PeerInfo, Box<dyn Error>> {
        let data = format!(r#"{{"ip":"{}","port":{}}}"#, ip, port);
        let response_byte = self
            .post(String::from("/connect-peer"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn get_work(&self) -> Result<GetWorkResp, Box<dyn Error>> {
        let response_byte = self
            .post(String::from("/get-work"), String::new())
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn submit_work(&self, block_header: &str) -> Result<bool, Box<dyn Error>> {
        let data = format!(r#"{{"block_header":"{}"}}"#, block_header);
        let response_byte = self.post(String::from("/submit-work"), data).await.unwrap();
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
    ...
    pub async fn get_work_json(&self) -> Result<GetWorkJSONResp, Box<dyn Error>> {
        let response_byte = self
    ...
    pub async fn submit_work_json(&self,param: &SubmitWorkJSONReq)-> Result<bool, Box<dyn Error>>{
        let data = serde_json::to_string(param)?;
        let response_byte = self
            .post(String::from("/get-work-json"), data)
            .await?;
        println!("response_byte {}",str::from_utf8(response_byte.bytes())?);
        let result: Response<bool> = serde_json::from_slice(response_byte.bytes())?;
        if result.is_success(){
    ...
    async fn post(&self, path: String, data: String) -> Result<Bytes, Box<dyn Error>> {
        let uri = Uri::builder()
